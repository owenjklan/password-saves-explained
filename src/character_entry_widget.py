...
class PasswordEntry(QWidget):
    ...
    class PasswordCharacterEntry(QWidget):
        ...
        def __init__(self, *args, **kwargs):
            ...
            logger.info("%s instance created. value = %s. currentPixmap = %s", self.__class__.__name__, self.value,
                        self.currentPixmap)
    ...
    def __init__(self, *args, **kwargs):
        ...
        for i in range(0, self.PWD_CHARS_COUNT):
            newPwdChar = self.PasswordCharacterEntry(parent=self)
            newPwdChar.redraw()
    ...
    def updateValue(self):
        self.value = 0
        for widgetIndex, pwdWidget in enumerate(self.pwdWidgets):
            posValue = pwdWidget.value << (self.PWD_CHARS_COUNT - widgetIndex - 1) * 4
            self.value += posValue
        logger.info("Value update: %s", self.value)